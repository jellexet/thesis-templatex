...
\[
(text{total block size} - \text{block header}) / (\text{chunk size} + \text{chunk header}) =   \text{number of 26 byte chunks in a block}
\]
...
In a conservative scenario with the new formatting, the network buffer would occupy \apporox\text{25\%} less space.